...
<main class="relative flex flex-col w-full max-w-[1280px] px-[75px] mx-auto mt-[50px] mb-[62px]">
    <div class="flex">
                ...
                <div class="accordion-content p-5 pt-0 flex flex-col gap-5">
                    ...
                    <div class="flex flex-col rounded-[20px] border border-[#E8EFF7] p-5 gap-5">
                        ...
                        <div class="flex items-center rounded-[20px] gap-[14px]">
                            ...
                            <div>
                                <p class="font-bold text-xl leading-[30px]">{{\Str::ucfirst($tier->class_type)}}</p>
                                <p class="text-garuda-grey mt-1">{{'Rp. ' . number_format($tier
                                    ->price, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
        ...
        <div id="Seats-Options" 
            ...
            <label 
            class="group relative flex w-[55px] h-[52.25px] shrink-0 
            @if($tier->class_type == 'business') 
                [&:nth-child(4n+2)]:mr-10 
            @else 
                [&:nth-child(6n+3)]:mr-[46px] 
            @endif"
            ...
            <input type="checkbox" name="seat" 
                ...
                @if($seat->is_available) checked @endif/>

            
                <img src="{{ asset('assets/images/icons/seat.svg') }}" 
                    class="absolute w-full h-full object-contain opacity-100 group-has-[:checked]:opacity-0 group-has-[:disabled]:opasity-0 transition-all duration-300" 
                    alt="seat">

                <img src="{{ asset('assets/images/icons/seat-choosed.svg') }}" 
                    class="absolute w-full h-full object-contain opacity-0 group-has-[checked]:opacity-100 group-has-[:disabled]:opasity-0 transition-all duration-300" 
                    alt="seat">
            
                <img src="{{ asset('assets/images/icons/seat-disabled.svg') }}" 
                    class="absolute w-full h-full object-contain opacity-0 group-has-[:disabled]:opasity-100 transition-all duration-300"
                    alt="Unavailable seat">


            <p class="relative flex items-center justify-center h-full w-full pb-[8.25px] font-semibold text-[16.5px] leading-[24.75px] 
                text-premiere-black group-has-[checked]:text-white">
                {{ $seat->name }}
            </p>
        </label>
    @endforeach
